...
{{--                    @if(auth()->user()->can('create-packages') || $user_role == $admin )--}}
                        <a href="{{ fr_route('carriers.create') }}" class="btn btn-primary m-1">{{ __('warehouse::view.add_carrier') }}</a>
{{--                    @endif--}}
                    <!--end::Add user-->
                </div>
                <!--end::Toolbar-->
...
@section('scripts')
    ...
    <script>
        function update_carrier_active(el){
            ...
            $.post('{{ route('carriers.update_active') }}', {_token:'{{ csrf_token() }}', id:id, active:active}, function(data){
                if(data == 1){
