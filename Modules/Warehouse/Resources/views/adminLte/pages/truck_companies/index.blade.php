...
{{--                    @if(auth()->user()->can('create-packages') || $user_role == $admin )--}}
                        <a href="{{ fr_route('truck_companies.create') }}" class="btn btn-primary m-1">{{ __('warehouse::view.add_truck_company') }}</a>
{{--                    @endif--}}
                    <!--end::Add user-->
                </div>
                <!--end::Toolbar-->
...
@section('scripts')
    ...
    <script>
        function update_truck_company_active(el){
            ...
            $.post('{{ route('truck_companies.update_active') }}', {_token:'{{ csrf_token() }}', id:id, active:active}, function(data){
                if(data == 1){
